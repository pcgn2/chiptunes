...
ChiptuneJsPlayer.prototype.getCurrentRow = function() {
  return Module._openmpt_module_get_current_row(this.currentPlayingNode.modulePtr);
}

ChiptuneJsPlayer.prototype.getCurrentPattern = function() {
  return Module._openmpt_module_get_current_pattern(this.currentPlayingNode.modulePtr);
}

ChiptuneJsPlayer.prototype.getCurrentOrder = function() {
  return Module._openmpt_module_get_current_order(this.currentPlayingNode.modulePtr);
}

ChiptuneJsPlayer.prototype.metadata = function() {
  var data = {};
  var keys = Pointer_stringify(Module._openmpt_module_get_metadata_keys(this.currentPlayingNode.modulePtr)).split(';');
  var keyNameBuffer = 0;
  for (var i = 0; i < keys.length; i++) {
    keyNameBuffer = Module._malloc(keys[i].length + 1);
    writeAsciiToMemory(keys[i], keyNameBuffer);
    data[keys[i]] = Pointer_stringify(Module._openmpt_module_get_metadata(this.currentPlayingNode.modulePtr, keyNameBuffer));
...
ChiptuneJsPlayer.prototype.module_ctl_set = function(ctl, value) {
  return Module.ccall('openmpt_module_ctl_set', 'number', ['number', 'string', 'string'], [this.currentPlayingNode.modulePtr, ctl, value]) === 1;
}
...
ChiptuneJsPlayer.prototype.play = function(buffer) {
  ...
  Module._openmpt_module_set_render_param(processNode.modulePtr, OPENMPT_MODULE_RENDER_STEREOSEPARATION_PERCENT, this.config.stereoSeparation);
  Module._openmpt_module_set_render_param(processNode.modulePtr, OPENMPT_MODULE_RENDER_INTERPOLATIONFILTER_LENGTH, this.config.interpolationFilter);
